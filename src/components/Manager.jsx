...
export const Manager = () => {
  ...
  return (
    ...
    {restaurants && restaurants.map((r, i) => (
        <NavLink key={i} to={`/restaurant/${r.id}`} className='flex p-4 justify-between border-b bg-slate-300 hover:bg-slate-500 hover:text-slate-200 items-center'>
            <h1>{r.name}</h1>
            <figcaption className='flex items-center'>
                <NavLink to={`/restaurant/e/edit/${r.id}`} className='p-2'><hi2.HiPencil className='h-5 w-5' /></NavLink>
                <button onClick={() => dispatch(actions.editRestaurant({req: 'delete', id: r.id}))} className='p-2'><hi2.HiArchiveBox className='h-5 w-5' /></button>
            </figcaption>
        </NavLink>
    ) )}
